...
//
// GetSymbolsFromElf64ExecutableInternal()
// ***************************************
//
int static GetSymbolsFromElf64ExecutableInternal (CONTEXT_REC * cr, char * modname, int module_type, bfd * abfd)
{
   Elf64_Ehdr * eh;                         // Elf Header
   Elf64_Phdr * ph_table, * ph;             // Program Header
   Elf64_Shdr * sh_table, * sh;             // Section Headers
   Elf64_Shdr * sym_sh = NULL;              // Symbol table section header
   Elf64_Shdr * dynsym_sh = NULL;           // Dynamic symbol table section header
   Elf64_Sym  * sym_table, * sym;           // Symbol Table/Section
   ...
   dbghv(("> GetSymbolsFromElf64ExecutableInternal: fn='%s', cr=%p, cr->load_addr=%p, module_type=%d, abfd=%p\n", modname, cr, cr->load_addr, module_type, abfd));
   //
   // Set beginning of the Program Header table and Section Header table
   //
   mn = (MOD_NODE *)cr->handle;
   eh = (Elf64_Ehdr *)cr->mod_addr;                       // Elf header is beginning of file
   ph_table = (Elf64_Phdr *)PtrAdd(eh, eh->e_phoff);      // Program headers table
   sh_table = (Elf64_Shdr *)PtrAdd(eh, eh->e_shoff);      // Section headers table

   mn->li.abfd = abfd;
   mn->li.asym = NULL;
   mn->li.asymcnt = 0;
   mn->flags |= A2N_FLAGS_64BIT_MODULE;                   // ELF (64-bit) module
   ...
   for (i = 0; i < eh->e_phnum; i++) {
      dbghv (("%02d 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x 0x%0x\n",
   ...
   for (i = 0, sh = sh_table;  i < eh->e_shnum;  i++, sh++) {
      ...
      if (sh->sh_flags & SHF_EXECINSTR) {
         if (escnt == MAX_EXEC_SECTIONS) {
            errmsg(("*E* GetSymbolsFromElf64ExecutableInternal: ***** INTERNAL ERROR: more than %d executable sections!\n", MAX_EXEC_SECTIONS));
            return (0);                           // No symbols
         }
         if (sh->sh_size > 0xFFFFFFFF || sh->sh_offset > 0xFFFFFFFF) {
            errmsg(("*E* GetSymbolsFromElf64ExecutableInternal: ***** INTERNAL ERROR: sh_size or sh_offset > 0xFFFFFFFF!\n"));
            errmsg(("                                   sh_size = 0x%X  sh_offset = 0x%X\n", sh->sh_size, sh->sh_offset));
            return (0);                           // No symbols
         }
         e_sec[escnt].number = i;
         e_sec[escnt].name = secname;

         e_sec[escnt].offset = PtrToUint32(PtrSub(sh->sh_addr, base_addr));

         if (module_type == MODULE_TYPE_ELF64_EXEC) {
            // Executables are images and sections do not require fixups
            e_sec[escnt].start_addr = sh->sh_addr;
         }
         else { // MODULE_TYPE_ELF64_REL
         ...
         e_sec[escnt].size = PtrToUint32(sh->sh_size);
         e_sec[escnt].end_addr    = e_sec[escnt].start_addr + sh->sh_size - 1;
         e_sec[escnt].file_offset = PtrToUint32(sh->sh_offset);
         e_sec[escnt].flags = PtrToUint32(sh->sh_flags);
         e_sec[escnt].type = sh->sh_type;

         e_sec[escnt].loc_addr = NULL;      // Until we decide if we need code

         dbghv(("%2x 0x%08x 0x%016llx 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x"  *X* '%s'\n",
      ...
      if (sh->sh_flags & SHF_ALLOC) {
         ...
         a_sec[ascnt].offset = PtrToUint32(PtrSub(sh->sh_addr, base_addr));
         ...
         a_sec[ascnt].size = PtrToUint32(sh->sh_size);
         a_sec[ascnt].end_addr    = a_sec[ascnt].start_addr + sh->sh_size - 1;
         a_sec[ascnt].file_offset = PtrToUint32(sh->sh_offset);
         a_sec[ascnt].flags = PtrToUint32(sh->sh_flags);
         a_sec[ascnt].type = sh->sh_type;

         a_sec[ascnt].loc_addr = NULL;   // Never code for these!

         dbghv(("%2x 0x%08x 0x%016llx 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x"  *A* '%s'\n",
                i, sh->sh_type, sh->sh_flags, sh->sh_link, sh->sh_info,
      ...
      if (sh->sh_type == SHT_SYMTAB) {
         if (symtab_section_cnt > 0) {
            errmsg(("*E* GetSymbolsFromElf64ExecutableInternal: ***** INTERNAL ERROR: more than 1 SYMTAB sections!\n"));
            return (0);                           // No symbols
         }
         NumberOfSymbols = sh->sh_size / sizeof(Elf64_Sym);
         sym_sh = sh;                       // Remember section header
         dbghv(("%2x 0x%08x 0x%016llx 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x"  *S* '%s' symcnt=%d\n",
                i, sh->sh_type, sh->sh_flags, sh->sh_link, sh->sh_info,
      ...
      dbghv(("%2x 0x%08x 0x%016llx 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%"_LZ64x"      '%s'\n",
             i, sh->sh_type, sh->sh_flags, sh->sh_link, sh->sh_info,
             sh->sh_addr, sh->sh_offset, 0xffffffff, sh->sh_size,
   ...
   for (i = 0; i < escnt; i++) {
      if (G(gather_code) && e_sec[i].type != SHT_NOBITS) {
         ...
         if (e_sec[i].loc_addr == NULL) {
            errmsg(("*E* GetSymbolsFromElf64ExecutableInternal: Unable to malloc(%d) for code for section %s\n", e_sec[i].size, e_sec[i].name));
         }
      }

      dbghv(("%2x 0x%08x 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x %"_PZp"  %s\n",
      ...
      if (e_sec[i].loc_addr != NULL) {
         dbghv(("- GetSymbolsFromElf64ExecutableInternal: Saving %d bytes of code for section %s\n", e_sec[i].size, e_sec[i].name));
         memcpy(e_sec[i].loc_addr,
                (void *)PtrAdd(eh, e_sec[i].file_offset),
                e_sec[i].size);
      }

      e_sec[i].sec = NULL;

      dbghv(("- GetSymbolsFromElf64ExecutableInternal: asction for %s = %p\n", e_sec[i].name, e_sec[i].sec));
      ...
      //
      if (i == plt_index) {
         dbghv(("- GetSymbolsFromElf64ExecutableInternal: Adding pseudo symbol for .plt\n"));
   ...
   for (i = 0; i < ascnt; i++) {
      dbghv (("%2x 0x%08x 0x%08x 0x%08x 0x%"_LZ64x" 0x%"_LZ64x" 0x%08x %"_PZp"  %s\n",
              a_sec[i].number, a_sec[i].type, a_sec[i].flags, a_sec[i].offset,
              a_sec[i].start_addr, a_sec[i].end_addr, a_sec[i].size,
              a_sec[i].loc_addr, a_sec[i].name));
   ...
   if ((mn->flags & A2N_FLAGS_VDSO) && dynsym_sh != NULL) {
      sym_table = (Elf64_Sym *)PtrAdd(eh, dynsym_sh->sh_offset);  // Beginning of symbol table
   ...
   if (NumberOfSymbols == 0) {
      dbghv(("< GetSymbolsFromElf64ExecutableInternal: file '%s' contains no symbols.\n", modname));
      return (0);                           // No symbols
   }
   ...
   //
   sym_table = (Elf64_Sym *)PtrAdd(eh, sym_sh->sh_offset);  // Beginning of symbol table
...
EnumerateSymbols:
   ...
   for (i = 0, sym = sym_table;  i < NumberOfSymbols;  i++, sym++) {
      ...
      if (sym->st_shndx == 0) {
         DumpElf64Symbol("NULL", "***Section index == 0***", sym);
         continue;
      }
      ...
      if (symbind != STB_LOCAL  &&  symbind != STB_GLOBAL  &&  symbind != STB_WEAK) {
         DumpElf64Symbol("NULL", "***Not function or label (BIND != LOCAL/GLOBAL/WEAK)***", sym);
      ...
      if (it_is_the_kernel) {
         ...
         if (symtype != STT_FUNC  &&  symtype != STT_NOTYPE  &&  symtype != STT_OBJECT) {
            DumpElf64Symbol(symname, "***Not function, label, or object (TYPE != FUNC/NOTYPE/OBJECT)***", sym);
            continue;                          // TYPE attribute not what we want
         }

         // Probably absolute symbol but value too small
         if (sym->st_value < base_addr) {
            DumpElf64Symbol(symname, "***Kernel and absolute symbol < base_addr*****", sym);
            continue;
         }
         ...
         if (symtype != STT_FUNC  &&  symtype != STT_NOTYPE) {
            DumpElf64Symbol(symname, "***Not function or label (TYPE != FUNC/NOTYPE)***", sym);
            continue;                          // TYPE attribute not what we want
         }

         // Symbol in undefined/reserved section
         if (sym->st_shndx == SHN_UNDEF  ||  sym->st_shndx > eh->e_shnum) {
            DumpElf64Symbol(symname, "***Undefined(0)/Reserved(>0xff00) Section***", sym);
         ...
         if ((sh->sh_type != SHT_PROGBITS) || !(sh->sh_flags & SHF_EXECINSTR)) {
            DumpElf64Symbol(symname, "**Non-executable section**", sym);
      ...
      // * .L# (where # is a number)
      // * L## (where # is a number and thing between them is a smily face)
      ...
      DumpElf64Symbol(symname, NULL, sym);
      ...
      // ***** Doesn't seem to work on IA64 or PPC so don't do it there *****
      //
#if (!defined(_IA64) && !defined(_PPC))
      ...
      if (rc) {
         dbghv(("< GetSymbolsFromElf64ExecutableInternal: SendBackSymbol returned %d. Stopping\n", rc));
   ...
   //
   dbghv(("< GetSymbolsFromElf64ExecutableInternal: Done enumerating. Found %d symbols.\n", NumberOfSymbolsKept));
...
int static GetSymbolsInternal(CONTEXT_REC * cr, char * modname, int module_type, bfd * abfd)
{
    return (GetSymbolsFromElf64ExecutableInternal(cr, modname, module_type, abfd));
}


//
// GetSymbolsFromElf64Executable()
// *******************************
//
// Harvest symbols from non-stripped ELF64 executables.
...
int GetSymbolsFromElf64Executable (CONTEXT_REC * cr, char * modname, int module_type, bfd * abfd)
{
	...
	mn->li.asymcnt = 0;
	mn->flags |= A2N_FLAGS_64BIT_MODULE;	// ELF (64-bit) module
	return (GetSymbolsInternal (cr, modname, module_type, abfd));
...
void DumpElf64Symbol(char *name, char * comment, Elf64_Sym * sym)
{
   ...
   if (comment == NULL) {                   // Good symbols
      ...
      int  symbind = ELF32_ST_BIND(sym->st_info);

      if (symtype == STT_NOTYPE) {
         dbghv(("Lbl  "));
		}
		else if (symbind == STB_WEAK) {
         dbghv(("WFn  "));
		}
		...
		dbghv (("0x%08x  0x%" _LZ64x "  0x%" _LZ64x
			"  0x%02x     0x%02x      0x%04x     %s\n", sym->st_name,
			sym->st_value, sym->st_size, sym->st_info, sym->st_other, sym->st_shndx, name));
   }
   else {                                   // Bad symbols
		dbghv (("     0x%08x  0x%" _LZ64x "  0x%" _LZ64x
			"  0x%02x     0x%02x      0x%04x     %s   %s\n",
